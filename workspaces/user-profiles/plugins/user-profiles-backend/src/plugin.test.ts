...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [userProfilesPlugin],
    });

    await request(server).get('/api/user-profiles/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/user-profiles/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/user-profiles/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/user-profiles/todos/${createdTodoItem.id}`)
